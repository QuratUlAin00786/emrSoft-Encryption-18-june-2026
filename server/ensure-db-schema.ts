...
export async function ensureDatabaseSchema(
  ...
  `).catch((e) =>
    console.warn("[DB] jazzcash_transactions CREATE skipped:", (e as Error)?.message || e),
  );

  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_jazzcash_transactions_invoice
      ON jazzcash_transactions (invoice_id)
  `).catch(() => undefined);

  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_jazzcash_transactions_org
      ON jazzcash_transactions (organization_id)
  `).catch(() => undefined);

  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_jazzcash_transactions_status
      ON jazzcash_transactions (status)
  `).catch(() => undefined);
